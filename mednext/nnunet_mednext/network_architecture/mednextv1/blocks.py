...
class DynamicTanh(nn.Module):
    def __init__(self, normalized_shape, alpha_init_value=0.5):
        ...
        self.weight1 = nn.Parameter(torch.ones(normalized_shape))     # 逐特征权重
        self.bias1 = nn.Parameter(torch.zeros(normalized_shape))      # 逐特征偏置
    def forward(self, x):
        x = torch.tanh(self.alpha * x)  # 动态缩放后的Tanh
        return x * self.weight1 + self.bias1  # 特征级校准
