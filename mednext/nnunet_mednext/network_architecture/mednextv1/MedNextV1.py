...
#             for i in range(1)])
#
#         # self.out =  nn.Sequential(*[nn.Linear(8*9*6*5, 1)])
#         self.out = nn.Linear(8 * 3 * 4 * 5, 1)
#
#         # Used to fix PyTorch checkpointing bug
#         self.dummy_tensor = nn.Parameter(torch.tensor([1.]), requires_grad=True)
#
...
#
#             x = self.iterative_checkpoint(self.bottleneck, x)
#             x = self.iterative_checkpoint(self.bottleneck2, x)
#
#             x = torch.reshape(x, (x.size()[0], x.size()[1]*x.size()[2]*x.size()[3]*x.size()[4]))
#
#             x = checkpoint.checkpoint(self.out, x)
#
#         else:
#             x_res_0 = self.enc_block_0(x)
#             x = self.down_0(x_res_0)
