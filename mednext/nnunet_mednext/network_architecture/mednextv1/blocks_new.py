...
class DynamicTanh(nn.Module):
    def __init__(self, normalized_shape, alpha_init_value=0.5):
        super().__init__()
        self.alpha = nn.Parameter(torch.ones(1) * alpha_init_value)  # 可学习缩放因子
        self.weight = nn.Parameter(torch.ones(normalized_shape))     # 逐特征权重
        self.bias = nn.Parameter(torch.zeros(normalized_shape))      # 逐特征偏置
    def forward(self, x):
        x = torch.tanh(self.alpha * x)  # 动态缩放后的Tanh
        return x * self.weight + self.bias  # 特征级校准
...
class MedNeXtBlock(nn.Module):
    ...
    def __init__(self, 
        ...
        # self.norm = DynamicTanh((1,in_channels,1,1,1))
        ...
        self.linear1_w = nn.Linear(3, in_channels)
        self.linear2_w = nn.Linear(in_channels, in_channels)
        self.linear1_b = nn.Linear(3, in_channels)
        self.linear2_b = nn.Linear(in_channels, in_channels)


    def forward(self, pdose, x, dummy_tensor=None):
        
        x1 = x
        x1 = self.conv1(x1)
        x1 = self.norm(x1)
        weight = self.act(self.linear2_w(self.act(self.linear1_w(pdose))))
        bias = self.act(self.linear2_b(self.act(self.linear1_b(pdose))))
        bs, c_num = weight.size()
        x1 = torch.reshape(weight, (bs, c_num, 1, 1, 1)) * x1 + torch.reshape(bias, (bs, c_num, 1, 1, 1))
...
class LayerNorm(nn.Module):
    ...
    """
    def __init__(self, normalized_shape, eps=1e-5, data_format="channels_last", affine=False):
        super().__init__()
        self.affine = affine
        if affine:
