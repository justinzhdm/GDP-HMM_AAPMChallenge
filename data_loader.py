...
# def crop_pad(In_dict, in_size, crop_center):
#     for key in In_dict.keys():
#         data = copy.deepcopy(In_dict[key])
#         in_size = [int(in_size[0]), int(in_size[1]), int(in_size[2])]
#         half_size = np.array(in_size) // 2
#         z_begin = int(max(0, crop_center[0] - half_size[0]))
#         z_end = int(min(data.shape[1], crop_center[0] + half_size[0] - 1))
#         y_begin = int(max(0, crop_center[1] - half_size[1]))
#         y_end = int(min(data.shape[2], crop_center[1] + half_size[1] - 1))
#         x_begin = int(max(0, crop_center[2] - half_size[2]))
#         x_end = int(min(data.shape[3], crop_center[2] + half_size[2] - 1))
#         bbox = [z_begin, data.shape[1]-1-z_end, y_begin, data.shape[2]-1-y_end, x_begin, data.shape[3]-1-x_end]
#         data = data[:, z_begin:z_end+1, y_begin:y_end+1, x_begin:x_end+1]
#         d, h, w = data.shape[1:]
#         z_pad1 = (in_size[0]-d)//2
#         z_pad2 = in_size[0]-d -(in_size[0]-d)//2
#         y_pad1 = (in_size[1]-h)//2
#         y_pad2 = in_size[1]-h -(in_size[1]-h)//2
#         x_pad1 = (in_size[2]-w)//2
#         x_pad2 = in_size[2]-w -(in_size[2]-w)//2
#         padding = [z_pad1, z_pad2, y_pad1, y_pad2, x_pad1, x_pad2]
#         data = np.pad(data, ((0,0), (z_pad1, z_pad2), (y_pad1, y_pad2), (x_pad1, x_pad2)), 'constant', constant_values=0)
#         assert data.shape[0]==1 and data.shape[1] == in_size[0] and data.shape[2] == in_size[1] and data.shape[3] == in_size[2]
#         In_dict[key] = data
#     return In_dict, bbox, padding

def resize(In_dict, KEYS, in_size):
    for key in KEYS:
        shape = In_dict[key].size()[1:]
        # data = F.interpolate(In_dict[key][None, :, :, :, :], scale_factor=(float(in_size[0]) / float(shape[0]),
        #                                                                    float(in_size[1]) / float(shape[1]),
        #                                                                    float(in_size[2]) / float(shape[2])), mode="trilinear", align_corners=False)[0]
        in_size = [int(in_size[0]), int(in_size[1]), int(in_size[2])]
        data = F.interpolate(In_dict[key][None, :, :, :, :].float(), size=in_size, mode="trilinear", align_corners=False)[0]
        # print(in_size, data.size(), shape)
        # raise
        In_dict[key] = data.float()
    return In_dict

# def crop_pad(In_dict, KEYS, in_size):
#     for key in KEYS:
#         data = copy.deepcopy(In_dict[key])
#         d, h, w = data.size()[1:]
#         z_pad1 = (in_size[0]-d)//2
...
#         x_pad2 = in_size[2]-w -(in_size[2]-w)//2
#         padding = [z_pad1, z_pad2, y_pad1, y_pad2, x_pad1, x_pad2]
#         data = F.pad(data, (x_pad1, x_pad2, y_pad1, y_pad2, z_pad1, z_pad2), 'constant', value=0)
#         assert data.size()[0]==1 and data.size()[1] == in_size[0] and data.size()[2] == in_size[1] and data.size()[3] == in_size[2]
#         In_dict[key] = data.float()
#     return In_dict, padding

def crop_pad(In_dict, KEYS):
    d_list = [96, 112, 128, 144, 160, 176]
    h_list = [80, 96, 112, 128, 144, 160]
    w_list = [144, 160, 176, 192, 208, 224, 240, 256, 272]

    d, h, w = In_dict['img'].size()[1:]
    new_d = None
    new_h = None
    new_w = None
    ...
    z_pad1 = (new_d - d) // 2
    z_pad2 = new_d - d - (new_d - d) // 2
    y_pad1 = (new_h - h) // 2
    y_pad2 = new_h - h - (new_h - h) // 2
    x_pad1 = (new_w - w) // 2
    x_pad2 = new_w - w - (new_w - w) // 2
    padding = [z_pad1, z_pad2, y_pad1, y_pad2, x_pad1, x_pad2]

    new_size = [new_d, new_h, new_w]
    # print(';new_size', new_size)

    for key in KEYS:
        data = copy.deepcopy(In_dict[key])
        data = F.pad(data, (x_pad1, x_pad2, y_pad1, y_pad2, z_pad1, z_pad2), 'constant', value=0)
        assert data.size()[0] == 1 and data.size()[1] == new_d and data.size()[2] == new_h and data.size()[3] == new_w
        In_dict[key] = data.float()

    return In_dict, padding, new_size
...
class MyDataset(Dataset):
    ...
    def __getitem__(self, index):
        ...
        if 'dose' in In_dict.keys():
            ...
            # print(np.max(In_dict['dose']), np.min(In_dict['dose']))
            ...
            origin_label = copy.deepcopy(In_dict['dose'])
            origin_body = copy.deepcopy(In_dict['Body'])
        ...
        # In_dict, bbox, padding = crop_pad(In_dict, self.cfig['in_size'], isocenter)
        # In_dict= resize(In_dict, KEYS, self.cfig['in_size'])
        # In_dict, padding = crop_pad(In_dict, KEYS, self.cfig['in_size'])
        In_dict, padding, new_size = crop_pad(In_dict, KEYS)
        ...
        # if self.phase in ['val', 'test', 'valid', 'external_test', 'train_test']:
        #     self.aug = tt_augmentation(KEYS, self.cfig['in_size'], self.cfig['out_size'], isocenter)
        ...
        prompt_extend = data_dict['prompt'][None, :, None, None].repeat(1, new_size[0] // 4, new_size[1], new_size[2])
        # print(comb_optptv.size())
        # print(comb_ptv.size())
        # print(comb_oar.size())
        # print(prompt_extend.size())
        # print(In_dict['Body'].size())
        # print(In_dict['img'].size())
        # print(data_dict['beam_plate'].size())
        # print(data_dict['angle_plate'].size())
        ...
        # print(np.max(In_dict['img']), np.min(In_dict['img']))
        # print(np.max(comb_optptv), np.min(comb_optptv))
        # print(np.max(comb_ptv), np.min(comb_ptv))
        # print(np.max(comb_oar), np.min(comb_oar))
        # print(np.max(data_dict['label']), np.min(data_dict['label']))
        # print(np.max(In_dict['Body']), np.min(In_dict['Body']))
        # print(np.max(In_dict['dose']), np.min(In_dict['dose']))
        ...
        data_dict['prescribed_dose'] = torch.tensor(prescribed_dose).float()
        ...
        if 'dose' in In_dict.keys():
            data_dict['origin_label'] = origin_label[None, :, :, :]
            data_dict['origin_body'] = origin_body[None, :, :, :]
        data_dict['padding'] = np.array(padding)
        # data_dict['bbox'] = np.array(bbox)
...
class GetLoader(object):
    ...
    def train_dataloader_test(self):
        dataset = MyDataset(self.cfig, phase='train_test')
        return DataLoader(dataset, batch_size=self.cfig['train_bs'], shuffle=False, num_workers=self.cfig['num_workers'], drop_last=True)
        
    def train_dataloader(self):
        dataset = MyDataset(self.cfig,  phase='train') 
        return DataLoader(dataset, batch_size=self.cfig['train_bs'],  shuffle=True, num_workers=self.cfig['num_workers'], drop_last=True)
    
    def val_dataloader(self):
        dataset = MyDataset(self.cfig, phase='valid') 
        return DataLoader(dataset, batch_size=self.cfig['val_bs'], shuffle=False, num_workers=self.cfig['num_workers'], drop_last=True)
